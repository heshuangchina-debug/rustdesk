...
#define YHLVNC_LIB_PATH "libyhlovnc.so"
typedef bool (*YhloVncCoreMain)();
bool gIsConnectionManager = false;

void print_help_install_pkg(const char* so);

bool flutter_yhlovnc_core_main() {
   void* libyhlovnc = dlopen(YHLVNC_LIB_PATH, RTLD_LAZY);
   if (!libyhlovnc) {
      fprintf(stderr,"Failed to load \"libyhlovnc.so\"\n");
      char* error;
   ...
   auto core_main = (YhloVncCoreMain) dlsym(libyhlovnc,"yhlovnc_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"yhlovnc_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
